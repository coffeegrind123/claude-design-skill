...
## Exporting to actual 1080×1080 PNGs for Instagram / X / LinkedIn

The HTML preview stages three 1080×1080 cards on a single page for
review. To export each one as a standalone 1080×1080 PNG that you can
drop straight into Instagram, build a per-card export file (or a
`?card=1` query param the page reads) and render each at the exact
canvas. Read `references/craft/headless-rendering.md` for the full
chrome-compensated render-then-crop pipeline — `--window-size=1080,
1080` does NOT give a 1080×1080 viewport in `--headless=new`. The
`fixed-canvas-card` recipe in this folder covers the same workflow.
