...
| `headless-rendering.md` | `headless-rendering` | Any skill that ships a PNG screenshot of an HTML artifact at exact target dimensions: `fixed-canvas-card` (Discord embed, OG image, X / LinkedIn share, IG square), `social-carousel`, `magazine-poster` when exported as an image. Covers the Chromium `--headless` viewport-compensation bug (the silent 87px clip), the render-then-crop pipeline, DOM-coordinate diagnostic overlays, PIL pixel sampling, and font-loading races. Not a brand or composition rule — it's a process discipline for the host→pixels round trip. |
